...
//			$endAge = 60;
			...
			$yearData = $this->findYears( $startAge, $endAge );
			$years = $yearData[1];
			$months = array();
	...
	//			$res['eight'] = $this->executeRuleEight();
				...
				$d = date(MarriageGuru::DATE_FORMAT,$time);
		...
		private function set_duration($start_age,$end_age)
		{
			...
			$this->_start_time = mktime(0,0,0,1,1,$this->_start_data['year']);
			
			$this->_end_time =  mktime(0,0,0,1,1,$this->_end_data['year']);					
		}
